...
class GRUDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, dictionary, encoder_embed_dim=512, embed_dim=512,
        ...
        self.layers = nn.ModuleList([
            GRUCell(encoder_embed_dim + embed_dim if layer == 0 else embed_dim, embed_dim)
            for layer in range(num_layers)
        ])
...
def GRUCell(input_dim, hidden_dim, **kwargs):
    m = nn.GRUCell(input_dim, hidden_dim, **kwargs)
...
def _check_arch(args):
    ...
    if args.arch not in get_archs():
        raise ValueError('Unknown GRU model architecture: {}'.format(args.arch))
    if args.arch != 'GRU':
...
def parse_arch(args):
    ...
    if args.arch == 'GRU_wiseman_iwslt_de_en':
    ...
    elif args.arch == 'GRU_luong_wmt_en_de':
...
def build_model(args, src_dict, dst_dict):
    ...
    return GRUModel(encoder, decoder)
